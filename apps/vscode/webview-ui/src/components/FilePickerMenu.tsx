  ...
  return (
    <div className="rounded-md border bg-popover shadow-md overflow-hidden">
      {mode === "search" ? (
        <>
          ...
          >
            ...
            <span className="text-xs">{t("filePicker.browseFolders")}</span>
          </button>
        </>
      ) : (
        <>
          ...
          >
            ...
            <span className="text-xs">{t("filePicker.backToSearch")}</span>
          </button>
          {currentPath && (
            <button
      ...
      <div className="max-h-64 overflow-y-auto">
        {isLoading ? (
          <div className="px-2 py-4 text-center text-xs text-muted-foreground">{t("common.loading")}</div>
        ) : items.length === 0 ? (
          <div className="px-2 py-4 text-center text-xs text-muted-foreground">{mode === "search" ? t("filePicker.noFilesFound") : t("filePicker.emptyFolder")}</div>
        ) : (
          items.map((item, idx) => {
