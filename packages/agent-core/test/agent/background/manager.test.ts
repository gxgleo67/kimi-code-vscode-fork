...
describe('BackgroundManager', () => {
  ...
  it('hides settled attached tasks unless includeAttached is set', async () => {
    const { manager } = createBackgroundManager();
    const attachedId = manager.registerTask(
      agentTask(Promise.resolve({ result: 'done' }), 'foreground agent'),
      { detached: false },
    );
    const detachedId = manager.registerTask(agentTask(Promise.resolve({ result: 'done' }), 'background agent'));
    await manager.wait(attachedId);
    await manager.wait(detachedId);

    expect(manager.list(false).map((info) => info.taskId)).toEqual([detachedId]);
    expect(manager.list(false, undefined, true).map((info) => info.taskId)).toEqual([attachedId, detachedId]);
    expect(manager.list(true, undefined, true)).toEqual([]);
  });
