...
<p align="center">
  <img src="docs/images/UI.jpg" alt="插件界面效果 | UI preview" width="80%">
</p>
...
产出 | Output：`apps/vscode/artifacts/vsix/kimi-code-<版本号>-universal.vsix`（默认打通用包，全平台可装；文件名带版本号，如 `kimi-code-0.9.2-universal.vsix`，多次打包不再互相覆盖。需要分平台包时执行 `node scripts/vsix-package.mjs all`）
...
[Apache-2.0](apps/vscode/LICENSE) — modified from Moonshot AI's original project. Original copyright belongs to Moonshot AI.
