 ...
 * The runtime harness is the v2 engine; a plain v1 harness shares the home for interop coverage.
...
describe("VS Code Kimi harness integration (shares one in-process SDK home)", () => {
  ...
  it("passes the management-plane tags through to the Webview payload", async () => {
    ...
    // Windows: v1 reports the raw join() (backslashes), v2 reports the
    // normalized forward-slash form — compare on the normalized shape.
    const normalized = servers.map((server) => ({ ...server, origin: server.origin?.replaceAll("\\", "/") }));
    expect(normalized).toEqual([
      {
        ...
        url: "https://example.test/mcp",
        source: "global",
        origin: join(rig.homeDir, "mcp.json").replaceAll("\\", "/"),
        mutable: true,
      },
    ]);
  });
  ...
  it("restores the persisted permission mode when a closed VS Code session reopens", async () => {
    const rig = await createRuntimeRig();
    const first = await openRuntimeSession(rig);
    await runSlash(first, "/yolo");
    await rig.runtime.detachView("view-1");

    // The session's stored mode wins; the global yolo setting no longer
    // forces a reopened session in either direction.
    const reopened = await openRuntimeSession(rig, first.id);
    expect(reopened.legacyApprovalFlags).toEqual({ yolo: true, afk: false });
