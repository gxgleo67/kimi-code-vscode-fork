...
export const Events = {
  ...
  LoginUrl: "loginUrl",
  SessionMetaUpdated: "sessionMetaUpdated",
} as const;
...
function validateParams(method: RpcMethod, params: unknown): boolean {
  switch (method) {
    ...
    case Methods.SetPermissionMode:
      return isPlainObject(params)
        && (params["mode"] === "manual" || params["mode"] === "yolo" || params["mode"] === "auto");
    ...
    case Methods.RespondApproval:
      return isPlainObject(params)
        ...
        && isApprovalResponse(params["response"]);
    ...
    case Methods.CreateGoal:
      return hasNonEmptyString(params, "objective");
    case Methods.ControlGoal:
      return isPlainObject(params)
        && (params["action"] === "pause" || params["action"] === "resume" || params["action"] === "cancel");
    case Methods.RenameKimiSession:
      return hasNonEmptyString(params, "sessionId") && hasNonEmptyString(params, "title");
...
function isApprovalResponse(value: unknown): boolean {
  if (value === "approve" || value === "approve_for_session" || value === "reject") return true;
  return isPlainObject(value)
    && (value["decision"] === "approve"
      || value["decision"] === "approve_for_session"
      || value["decision"] === "reject")
    && isOptionalType(value["feedback"], "string")
    && isOptionalType(value["selectedLabel"], "string");
}
...
function isStreamChatParams(value: unknown): boolean {
  return isPlainObject(value)
    ...
    && isOptionalType(value["goalObjective"], "string")
    && isOptionalPermissionMode(value["permissionMode"]);
}

function isOptionalPermissionMode(value: unknown): boolean {
  return value === undefined || value === "manual" || value === "yolo" || value === "auto";
}
...
/** `undefined` leaves the recipe alone, `null` clears it (follow the main model). */
function isSecondaryModelSelection(value: unknown): boolean {
  return value === undefined || value === null || (
    isPlainObject(value)
    && isNonEmptyString(value["model"])
    && isOptionalType(value["defaultEffort"], "string")
  );
}
