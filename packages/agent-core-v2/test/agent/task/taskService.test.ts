...
describe('AgentTaskService', () => {
  ...
  it('list hides settled attached tasks unless includeAttached is set', async () => {
    const svc = ix.get(IAgentTaskService);
    const attachedId = svc.registerTask(outputtingTask('x'), { detached: false });
    const detachedId = svc.registerTask(outputtingTask('x'));
    await svc.wait(attachedId);
    await svc.wait(detachedId);

    expect(svc.list(false).map((task) => task.taskId)).toEqual([detachedId]);
    expect(svc.list(false, undefined, true).map((task) => task.taskId)).toEqual([attachedId, detachedId]);
    expect(svc.list(true, undefined, true)).toEqual([]);
  });
