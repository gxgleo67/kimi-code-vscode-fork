...
export const ProviderConfigSchema = z.object({
  ...
  // Indirect secret reference: when `api_key` is unset, the API key is looked
  // up from `process.env[api_key_env_var]` at provider resolution time, so
  // config.toml holds only the variable name, never the secret (the MCP
  // `bearerTokenEnvVar` precedent).
