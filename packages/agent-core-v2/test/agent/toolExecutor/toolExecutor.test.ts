...
describe('Read through the truncation pipeline', () => {
  let homeDir: string;
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'tool-executor-read-'));
    const pathClass = process.platform === 'win32' ? 'win32' : 'posix';
    const services = disposables.add(new TestInstantiationService());
    services.stub(ILogService, stubLog());
    services.stub(IBootstrapService, stubBootstrap(homeDir));
    services.stub(
      IAgentScopeContext,
    ...
    const truncation = services.createInstance(ToolResultTruncationService);
    ...
    readConfig = services.get(IConfigService);
    ...
    const runtime = Object.assign(
      new FakeRuntime(
        ...
        { capabilities: ['fs', 'process'], pathClass, environment: { pathClass, homeDir } },
      ),
  ...
  it('recovers an expanded Glob listing beyond spill retention using complete saved pages', async () => {
    ...
    try {
      ...
      do {
        ...
        expect(page.output).toContain('showing a preview only');
      ...
      expect(recovered).toEqual(names.map((name) => `${rootPosix}/${name}`));
    } finally {
  ...
  it('resolves attachment references for media reads and exposes binary paths for converters', async () => {
    ...
    const refs = [...new Set([...text.matchAll(/Attachment reference: ("[^\n]+")/g)].map((match) => JSON.parse(match[1]!) as string))];
    const paths = [...new Set([...text.matchAll(/Original attachment saved at: ("[^\n]+")/g)].map((match) => JSON.parse(match[1]!) as string))];
