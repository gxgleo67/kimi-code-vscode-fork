...
export class BackgroundManager {
   ...
   */
  list(activeOnly = true, limit?: number, includeAttached = false): BackgroundTaskInfo[] {
    const result: BackgroundTaskInfo[] = [];
    for (const entry of this.tasks.values()) {
      const info = this.toInfo(entry);
      if (!this.shouldListTask(info, activeOnly, includeAttached)) continue;
      result.push(info);
      if (limit !== undefined && result.length >= limit) return result;
    }
    if (!activeOnly) {
      for (const ghost of this.ghosts.values()) {
        if (!this.shouldListTask(ghost, activeOnly, includeAttached)) continue;
        result.push(ghost);
        if (limit !== undefined && result.length >= limit) return result;
      }
    }
  ...
  private shouldListTask(info: BackgroundTaskInfo, activeOnly: boolean, includeAttached: boolean): boolean {
    if (!TERMINAL_STATUSES.has(info.status)) return true;
    if (activeOnly) return false;
    // Attached (foreground) tasks deliver their result to the waiting tool
    // call inline, so listing surfaces hide them once settled unless the
    // caller explicitly wants a session-wide activity view.
    if (includeAttached) return true;
