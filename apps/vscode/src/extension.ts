...
const LEGACY_REAUTH_NOTICE_KEY = "kimifork.legacyMigration.reauthNotice.v1";
const LEGACY_WARNING_NOTICE_KEY = "kimifork.legacyMigration.warningNotice.v1";

export async function activate(context: vscode.ExtensionContext): Promise<void> {
  outputChannel = vscode.window.createOutputChannel("Kimi Fork");
  ...
  // Custom-provider API keys live in SecretStorage; mirror them into
  // process.env before the embedded engine (constructed inside
  // KimiWebviewProvider) reads config.toml's api_key_env_var references.
  try {
    await injectProviderSecrets(context.secrets, log);
  } catch (error) {
    logError("Unable to inject custom provider secrets", error);
  }
  context.subscriptions.push(
    context.secrets.onDidChange((event) => {
      if (!event.key.startsWith(SECRET_KEY_PREFIX)) return;
      syncProviderSecret(context.secrets, event.key, log).catch((error: unknown) => {
        logError("Unable to sync a custom provider secret", error);
      });
    }),
  );
  ...
  context.subscriptions.push(
    vscode.workspace.registerTextDocumentContentProvider("kimifork-baseline", {
  ...
  context.subscriptions.push(
    ...
    vscode.window.registerWebviewViewProvider("kimifork.webview", provider, {
  ...
  const commands: Record<string, () => void | Promise<void>> = {
    "kimifork.clearAllState": async () => {
      await context.globalState.update("kimifork.config", undefined);
      await context.globalState.update("kimifork.mcpServers", undefined);
      await context.workspaceState.update("kimifork.mcpEnabled", undefined);
      await vscode.window.showInformationMessage("Kimi: Extension UI state cleared.");
    },
    "kimifork.openInTab": () => {
      provider?.createPanel();
    },
    "kimifork.openInSideBar": async () => {
      await vscode.commands.executeCommand("kimifork.webview.focus");
    },
    "kimifork.focusInput": async () => {
      await vscode.commands.executeCommand("kimifork.webview.focus");
      provider?.broadcast(Events.FocusInput, {});
    },
    "kimifork.insertMention": async () => {
    ...
    "kimifork.newConversation": async () => {
      await vscode.commands.executeCommand("kimifork.webview.focus");
      provider?.broadcast(Events.NewConversation, {});
    },
    "kimifork.showLogs": () => outputChannel?.show(),
    "kimifork.resetKimi": () => provider?.resetAllWebviews(),
    "kimifork.logout": async () => {
      await vscode.commands.executeCommand("kimifork.webview.focus");
      await vscode.window.showInformationMessage("Use the logout button in Kimi settings.");
    },
    "kimifork.migrateLegacyData": () => runMigration(true),
  };
