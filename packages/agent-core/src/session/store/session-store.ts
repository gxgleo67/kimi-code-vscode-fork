...
export class SessionStore {
  ...
  private async listWorkDir(
    ...
    for (const entry of entries) {
      ...
      const id = entry.name;
      if (!isSafeSessionId(id)) continue;
      candidates.push({ id, dir: join(bucketDir, id) });
    }

    // Summaries need several fs stats each — fan them out instead of
    // serializing hundreds of round-trips behind one another.
    const sessions: SessionSummary[] = [];
    await mapWithConcurrency(candidates, LIST_CONCURRENCY, async ({ id, dir }) => {
    ...
    const index = await readSessionIndex(this.homeDir, this.sessionsDir);
    await mapWithConcurrency([...index.values()], LIST_CONCURRENCY, async (entry) => {
      if (seen.has(entry.sessionId) || !(await isDirectory(entry.sessionDir))) return;
      const summary = await this.summaryFromDir(entry.sessionId, entry.sessionDir, entry.workDir);
      if (!areSameFsPath(summary.workDir, workDir)) return;
      if (!includeArchive && summary.archived === true) return;
      sessions.push(summary);
    });
  ...
  private async listAll(includeArchive: boolean): Promise<readonly SessionSummary[]> {
    ...
    const sessions: SessionSummary[] = [];
    await mapWithConcurrency([...index.values()], LIST_CONCURRENCY, async (entry) => {
      if (!(await isDirectory(entry.sessionDir))) return;
      const summary = await this.summaryFromDir(entry.sessionId, entry.sessionDir, entry.workDir);
      if (!includeArchive && summary.archived === true) return;
      sessions.push(summary);
    });
