...
const readPlanFile: Handler<FilePathParams, string> = async ({ filePath }) => {
  // Plan files live outside the workspace (<homedir>/plans or <cwd>/plan), so
  // the workspace-containment helpers cannot apply. Keep the surface narrow:
  // plan files are always Markdown, and plans are small.
  if (!filePath.toLowerCase().endsWith(".md")) throw new Error("Not a plan file.");
  const uri = vscode.Uri.file(filePath);
  const stat = await vscode.workspace.fs.stat(uri);
  if (stat.size > PLAN_FILE_MAX_BYTES) throw new Error("Plan file is too large to preview.");
