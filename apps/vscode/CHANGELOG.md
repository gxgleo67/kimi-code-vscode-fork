...
5. Account Management: hovering an account's quota line now shows a tooltip with each window's details — percent used, the localized reset countdown, and the exact reset timestamp.
...
5. 账号管理:鼠标悬停账号的额度行,弹出该账号各窗口的详情——已用百分比、本地化重置倒计时和精确重置时间
