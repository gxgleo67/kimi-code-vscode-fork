...
describe("session runtime (adapts one SDK session for subscribed Webviews)", () => {
  ...
  it("forwards plan review approvals to the Webview in legacy yolo mode", async () => {
    const { runtime, sdk, broadcasts } = createRuntime({ yolo: true, afk: false });
    const pending = sdk.requestApproval({
      toolCallId: "tool-plan-yolo",
      toolName: "ExitPlanMode",
      action: "Exit plan mode",
      display: {
        kind: "plan_review",
        plan: "# Plan\n\n1. Step one",
        path: "/home/sessions/session-1/plans/plan-1.md",
        options: [
          { label: "Big bang", description: "Migrate everything at once" },
          { label: "Incremental (Recommended)", description: "Migrate in phases" },
        ],
      },
    });
    let resolved = false;
    ...
    const request = streamData(broadcasts).find(
      (event) =>
        ...
        "type" in event &&
        event.type === "ApprovalRequest",
    ) as { payload: { id: string; display: unknown } };

    // YOLO never auto-approves a plan review: the request reaches the Webview
    // carrying the full plan and options, and stays pending until the user
    // decides.
    expect(request.payload.display).toEqual([
      {
        type: "plan_review",
        plan: "# Plan\n\n1. Step one",
        path: "/home/sessions/session-1/plans/plan-1.md",
  ...
  it("maps a plan revision response to a rejected SDK approval with feedback", async () => {
    const { runtime, sdk, broadcasts } = createRuntime();
    const pending = sdk.requestApproval({
      toolCallId: "tool-plan-revise",
      toolName: "ExitPlanMode",
      action: "Exit plan mode",
      display: { kind: "plan_review", plan: "# Plan", path: "/plans/plan.md" },
    });
    const request = streamData(broadcasts).find(
      (event) =>
        typeof event === "object" &&
    ...
    expect(
      runtime.respondApproval(request.payload.id, {
        decision: "reject",
  ...
  it("passes the selected plan option label through to the SDK approval", async () => {
    const { runtime, sdk, broadcasts } = createRuntime();
    const pending = sdk.requestApproval({
      toolCallId: "tool-plan-option",
      toolName: "ExitPlanMode",
      action: "Exit plan mode",
      display: {
        kind: "plan_review",
    ...
    expect(
      runtime.respondApproval(request.payload.id, {
        decision: "approve",
        selectedLabel: "Incremental (Recommended)",
      }),
    ).toBe(true);
    await expect(pending).resolves.toEqual({
      decision: "approved",
  ...
  it("applies a permission mode selection and persists it as legacy flags", async () => {
    const { runtime, sdk } = createRuntime();

    await runtime.setPermissionMode("auto");

    expect(sdk.setPermissions).toEqual(["auto"]);
    expect(sdk.metadataUpdates).toEqual([
      { vscode_legacy_approval: { yolo: false, afk: true } },
    ]);
    expect(runtime.legacyApprovalFlags).toEqual({ yolo: false, afk: true });
  });

  it("rolls the permission mode back when it cannot be persisted", async () => {
    const { runtime, sdk } = createRuntime();
    sdk.rejectNextMetadataUpdate(new Error("state is read-only"));

    await expect(runtime.setPermissionMode("yolo")).rejects.toThrow("state is read-only");

    expect(sdk.setPermissions).toEqual(["yolo", "manual"]);
    expect(runtime.legacyApprovalFlags).toEqual({ yolo: false, afk: false });
  });

  it("announces the session's permission mode to a subscribing view", async () => {
    const { runtime, broadcasts } = createRuntime();
    await runtime.setPermissionMode("auto");
  ...
  it("broadcasts session meta updates to every subscribed Webview", () => {
    const { runtime, sdk, broadcasts } = createRuntime();
    runtime.subscribe("view-2");
    ...
    sdk.emit({
      ...
      sessionId: "session-1",
      agentId: "main",
      title: "Refactor Login Page Hooks",
      patch: { title: "Refactor Login Page Hooks", isCustomTitle: false, lastPrompt: "..." },
    });

    const metaBroadcasts = broadcasts.filter((record) => record.event === Events.SessionMetaUpdated);
    expect(metaBroadcasts).toEqual([
      {
        event: Events.SessionMetaUpdated,
        data: { sessionId: "session-1", title: "Refactor Login Page Hooks" },
        webviewId: "view-1",
      },
      {
        event: Events.SessionMetaUpdated,
    ...
    // Meta updates are not turn traffic: nothing leaks into the stream channel.
    expect(streamData(broadcasts)).toEqual([]);
  });
});

describe("session runtime cancel fallback (unresponsive turns)", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
  ...
  function completionsOf(records: readonly BroadcastRecord[]): unknown[] {
    return streamData(records).filter(
      (event) =>
        typeof event === "object" && event !== null && "type" in event && event.type === "stream_complete",
    );
  }

  it("forces a cancelled terminal when the engine never answers a cancel", async () => {
    const { runtime, sdk, broadcasts } = createRuntime();
    const completion = runtime.prompt("hello");
    sdk.emit(turnStarted());

    await runtime.cancel();
    expect(completionsOf(broadcasts)).toEqual([]);
    ...
    // A late turn.ended for the same turn shares the forced terminal's key
    // and must not unlock the Webview a second time.
    sdk.emit(turnEnded("cancelled"));
    expect(completionsOf(broadcasts)).toHaveLength(1);
  });

  it("lets the real terminal win when the turn settles right after cancel", async () => {
    const { runtime, sdk, broadcasts } = createRuntime();
    const completion = runtime.prompt("hello");
    sdk.emit(turnStarted());
  ...
  it("arms no fallback when nothing is running", async () => {
    const { runtime, sdk, broadcasts } = createRuntime();
