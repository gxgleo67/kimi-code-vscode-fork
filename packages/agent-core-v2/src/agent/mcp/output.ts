...
export async function mcpResultToExecutableOutput(
  ...
  for (const block of result.content) {
    ...
    if (part === null && block.type === 'resource' &&
  ...
  return {
    ...
    note: notices.suffix.length > 0 ? notices.suffix : undefined,
    truncated: budgeted.truncated || capped.truncated || omittedAttachment ? true : undefined,
  };
}
