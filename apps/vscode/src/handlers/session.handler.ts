...
export const sessionHandlers: Record<string, Handler<any, any>> = {
  ...
  [Methods.RenameKimiSession]: async (params: RenameSessionParams, ctx): Promise<{ ok: boolean }> => {
    ...
    if (!isSessionId(params.sessionId) || !ctx.workspaceRoot || title.length === 0) return { ok: false };
    ...
    // Active sessions already emit session.meta.updated to their subscribers;
    // this broadcast also reaches every other open view and covers renames of
    // settled sessions, which emit nothing.
    ctx.broadcast(Events.SessionMetaUpdated, { sessionId: params.sessionId, title });
    return { ok: true };
  },
