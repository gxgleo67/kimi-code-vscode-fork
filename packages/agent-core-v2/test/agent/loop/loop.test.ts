...
describe('Agent loop', () => {
  ...
  it('runs a text-only agent turn from prompt to completion', async () => {
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [wire] prompt.accepted             { "promptId": "<msg-1>", "content": [ { "type": "text", "text": "Hello" } ], "time": "<time>" }
      [emit] prompt.accepted             { "time": "<time>", "promptId": "<msg-1>", "content": [ { "type": "text", "text": "Hello" } ] }
      [wire] turn.prompt                 { "input": [ { "type": "text", "text": "Hello" } ], "origin": { "kind": "user" }, "promptId": "<msg-1>", "turnId": 0, "time": "<time>" }
  ...
  it('runs an agent turn through registered tool approval and execution', async () => {
    ...
    expect(await ctx.untilApproval(true)).toMatchInlineSnapshot(`
      ...
      [wire] prompt.accepted                 { "promptId": "<msg-1>", "content": [ { "type": "text", "text": "Look up moon" } ], "time": "<time>" }
      [emit] prompt.accepted                 { "time": "<time>", "promptId": "<msg-1>", "content": [ { "type": "text", "text": "Look up moon" } ] }
      [wire] turn.prompt                     { "input": [ { "type": "text", "text": "Look up moon" } ], "origin": { "kind": "user" }, "promptId": "<msg-1>", "turnId": 0, "time": "<time>" }
...
describe('service-internal attempt retry', () => {
  it('discards the failed attempt stream when the requester retries below the turn', async () => {
    const requester: IAgentLLMRequesterService = {
      _serviceBrand: undefined,
      prepareTurnConfig: () => ({ thinkingEffort: 'off' }),
      async request(overrides, onPart = () => {}, signal) {
        await onPart({ type: 'text', text: 'stale attempt' });
        overrides?.onAttemptRetry?.();
        await onPart({ type: 'text', text: 'fresh attempt' });
    ...
    try {
      ...
      const subscription = ctx.get(IEventBus).subscribe(AssistantDelta, () => {
        deltas += 1;
        if (deltas === 2) loop.cancel();
      });
      const turn = (await loop.enqueue(nextTurnMessage('Hello')).assigned).turn;
      await expect(turn.result).resolves.toMatchObject({ type: 'cancelled' });
      subscription.dispose();

      const assistant = ctx.contextData().history.find((message) => message.role === 'assistant');
