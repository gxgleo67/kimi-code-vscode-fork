...
function providerApiKey(provider: ProviderConfig): string | undefined {
  // Resolution order: plaintext `api_key` → the `api_key_env_var` indirect
  // reference into process.env → the per-type key of the `[env]` subtable.
  const envVarRef = apiKeyEnvVarValue(provider);
  switch (provider.type) {
    case 'anthropic':
      return providerValue(provider.apiKey, provider.env, 'ANTHROPIC_API_KEY', envVarRef);
    case 'openai':
    case 'openai_responses':
      return providerValue(provider.apiKey, provider.env, 'OPENAI_API_KEY', envVarRef);
    case 'kimi':
      return providerValue(provider.apiKey, provider.env, 'KIMI_API_KEY', envVarRef);
    case 'google-genai':
      return providerValue(provider.apiKey, provider.env, 'GOOGLE_API_KEY', envVarRef);
    case 'vertexai':
      return (
...
// Reads the indirect secret reference: `api_key_env_var` names a process.env
// variable whose current value is the API key. An unset/empty reference or an
// unset variable both yield undefined, falling through to the next source.
function apiKeyEnvVarValue(provider: ProviderConfig): string | undefined {
  const name = nonEmptyString(provider.apiKeyEnvVar);
  return name === undefined ? undefined : nonEmptyString(process.env[name]);
}
...
function providerValue(
  ...
  return nonEmptyString(configured) ?? envVarRef ?? envValue(env, envKey);
}
