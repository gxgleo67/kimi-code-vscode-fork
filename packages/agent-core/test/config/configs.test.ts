...
`;
  ...
  it('parses a provider api_key_env_var indirect secret reference', async () => {
    const dir = makeTempDir();
    const configPath = join(dir, 'api-key-env-var.toml');
    const toml = `
[providers.deepseek]
type = "openai"
...
`;
    ...
    const roundTripped = parseConfigString(text, configPath);
    expect(roundTripped.providers['deepseek']?.apiKeyEnvVar).toBe('KIMIFORK_PROVIDER_KEY_DEEPSEEK');
  });
