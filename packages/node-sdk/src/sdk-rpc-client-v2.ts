...
export class SDKRpcClientV2 extends SDKRpcClientBase {
  ...
  override async listBackgroundTasks(
    input: SessionIdRpcInput & { activeOnly?: boolean; limit?: number; includeAttached?: boolean },
  ): Promise<readonly BackgroundTaskInfo[]> {
    const agent = await this.agentFacade(input.sessionId);
    return agent.getTasks({ activeOnly: input.activeOnly, limit: input.limit, includeAttached: input.includeAttached }) as Promise<
