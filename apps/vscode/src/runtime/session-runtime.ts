...
import {
  ...
  legacyApprovalForCorePermission,
...
export class SessionRuntime {
  ...
  /** Set the session's permission mode (manual/yolo/auto) and persist it. */
  async setPermissionMode(mode: PermissionMode): Promise<void> {
    await this.applyLegacyApproval(legacyApprovalForCorePermission(mode));
  }
   ...
   * plan mode, permission mode) matches engine truth instead of the global
   * defaults.
   */
  ...
  async cancel(): Promise<void> {
    ...
    // A stuck tool may never answer the cancel, so turn.ended never arrives
    // and subscribed views stay locked. Arm a fallback that forces the
    // terminal event; the real terminal (if any) disarms it on arrival.
  ...
  private async applyLegacyApproval(flags: LegacyApprovalFlags): Promise<void> {
    ...
    // Slash-command toggles (/yolo, /afk) take this path too — without an
    // announcement the composer picker would keep showing the previous mode
    // until the next attach.
    for (const webviewId of this.webviewIds) {
      await this.announceStatus(webviewId);
    }
  }
  ...
  private onSdkEvent(event: Event): void {
    ...
    if (event.type === "session.meta.updated") {
      // Title refreshes (easy title on the first prompt, llm-generated title,
      // renames) go to every subscribed view so the session list can update
      // live. The stream adapter below ignores this event type.
      for (const webviewId of this.webviewIds) {
        this.broadcast(
          Events.SessionMetaUpdated,
          { sessionId: event.sessionId, title: event.title },
          webviewId,
        );
      }
    }
  ...
  private armCancelFallback(): void {
    ...
    this.cancelFallbackTimer = setTimeout(() => {
      ...
      if (this.closed || this.activePrompt === undefined) return;
      this.log("Turn did not settle after cancel; forcing a terminal event");
      this.emitTerminal({
        // Reuse the real terminal key when the turn id is known so a late
        // turn.ended for this turn is de-duplicated by terminalKeys.
        key: `${this.session.id}:main:${turnId ?? "cancel-fallback"}`,
