...
describe('SessionMediaStoreService', () => {
  ...
  it.each([
    ...
    const message: Message = { role: 'tool', toolCallId: 'audio', content, toolCalls: [] };
    const body = await captureProviderRequest(provider as 'openai' | 'anthropic', message);
    expect(JSON.stringify(body)).toContain(JSON.stringify(encodedPath!).slice(1, -1));
...
const providerRegistry = new ProtocolAdapterRegistry();

async function captureProviderRequest(
  protocol: 'openai' | 'anthropic',
  message: Message,
): Promise<Record<string, unknown>> {
  const provider: ChatProvider = providerRegistry.createChatProvider({
    protocol,
    modelName: protocol === 'openai' ? 'gpt-4o' : 'claude-opus-4-6',
    apiKey: 'sk-probe',
  });
  ...
  const createChatCompletion = (params: unknown) => {
  ...
  const createMessage = (params: unknown) => {
    captured = params as Record<string, unknown>;
    return Promise.resolve(
      (async function* () {
        yield {
          type: 'message_start',
          message: { id: 'msg_probe', usage: { input_tokens: 1, output_tokens: 1 } },
        };
        yield {
          type: 'message_delta',
  ...
  if (protocol === 'openai') {
    client.chat.completions.create = createChatCompletion;
  } else {
    client.messages.create = createMessage;
    client.beta.messages.create = createMessage;
  }
  const stream = await provider.generate('', [], [message]);
  for await (const part of stream) void part;
  if (captured === undefined) throw new Error('expected a provider request');
  return captured;
}
