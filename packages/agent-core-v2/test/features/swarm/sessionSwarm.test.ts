...
describe('SessionSwarmService metadata compatibility', () => {
  ...
  it('emits subagent.cancelled (not subagent.failed) when a running child aborts', async () => {
    ...
    runAgent.mockImplementation((agentId: string) => ({
      agentId,
      turn: {} as never,
  ...
  it('emits neither subagent.cancelled nor subagent.failed on the rate-limit requeue path', async () => {
    ...
    try {
      ...
      let rlRuns = 0;
      runAgent.mockImplementation((agentId: string, _request: unknown, options: { onReady?: () => void }) => {
        options?.onReady?.();
        if (agentId === 'agent-rl') {
          rlRuns += 1;
          return {
  ...
  it('emits subagent.cancelled exactly once when a starting retry is cancelled', async () => {
    ...
    try {
      ...
      let rlRuns = 0;
      runAgent.mockImplementation((agentId: string, _request: unknown, options: { onReady?: () => void; signal: AbortSignal }) => {
        if (agentId === 'agent-rl') {
          rlRuns += 1;
          if (rlRuns === 1) {
            options?.onReady?.();
  ...
  it('emits subagent.cancelled exactly once when a child with an installed mirror is cancelled before ready', async () => {
    ...
    try {
      ...
      runAgent.mockImplementation((agentId: string, _request: unknown, options: { signal: AbortSignal }) => ({
        agentId,
        turn: {} as never,
  ...
  it('emits subagent.cancelled for a child whose spawn finishes after the batch is cancelled', async () => {
    vi.useFakeTimers();
    try {
      const newHandle = agentHandle('agent-new', lifecycle, eventBus, {
        profileName: 'coder',
        modelAlias: 'kimi-test',
      });
      handles.set('agent-new', newHandle);
      const spawnDeferred = createControlledPromise<IAgentScopeHandle>();
      createAgent.mockReturnValueOnce(spawnDeferred);
      runAgent.mockImplementationOnce((agentId: string, _request: unknown, options: { signal: AbortSignal }) => {
      ...
      spawnDeferred.resolve(newHandle);
      await vi.advanceTimersByTimeAsync(0);
