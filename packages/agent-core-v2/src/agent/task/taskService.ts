...
export class AgentTaskService extends Disposable implements IAgentTaskService {
  ...
  /** One-shot injection telling the model its previous-session background tasks lost contact on app exit. */
  private appendPreviousSessionTasksReminder(): void {
    ...
    this.context.append({
      ...
      toolCalls: [],
      origin: { kind: 'injection', variant: TASK_RESUME_TERMINATION_VARIANT },
    });
  ...
  list(activeOnly = true, limit?: number, includeAttached = false): readonly AgentTaskInfo[] {
    const result: AgentTaskInfo[] = [];
    for (const entry of this.tasks.values()) {
      const info = this.toInfo(entry);
      if (!shouldListTask(info, activeOnly, includeAttached)) continue;
      result.push(info);
      if (limit !== undefined && result.length >= limit) return result;
    }
    if (!activeOnly) {
      for (const ghost of this.ghosts.values()) {
        if (!shouldListTask(ghost, activeOnly, includeAttached)) continue;
        result.push(ghost);
        if (limit !== undefined && result.length >= limit) return result;
      }
    }
...
function shouldListTask(info: AgentTaskInfo, activeOnly: boolean, includeAttached: boolean): boolean {
...
function buildAgentTaskNotification(
  ...
  return {
    id: `task:${info.taskId}:${info.status}`,
    category: 'task',
