...
- 新增「精简模式」开关(设置菜单或 kimifork.compactComposer):输入框的权限/模式按钮在任意宽度下以图标形式精简显示;模型名称与思考强度改为永不截断、永不收成图标,窄侧栏(约 300px)下也始终完整可读——原先 560px 以下模型名被省略号截断、340px 以下整个收成 CPU 图标
- 错误重试改为手动:移除内联错误卡片上的「重试」按钮(程序化重发曾导致思考记录丢失、消息对交换错乱),超时/报错后请在输入框中重新输入指令;错误卡片新增提示文案,重试相关的 pendingRetry 状态机整体移除

### 2026-08-21

- 计划文件可读性:计划模式提醒与 ExitPlanMode 工具说明注入文档编排要求(标题 + 摘要 + 短章节 + 要点列表 + 代码/路径反引号 + 编号步骤),输出计划不再是大段文字
- 修复切窗/重载后当前对话从界面消失:webview 重建会拿到全新随机 webviewId 且宿主不推送当前会话,导致处理中内容清空、只剩"处理中..."标签和可用的发送按钮;现将会话 id 持久化到 localStorage,挂载时自动重附着并回放会话(历史加载动画遮盖),进行中回合恢复后直接回到流式状态;会话不可恢复时静默清除记录

- 子代理自定义供应商支持编辑:列表新增铅笔按钮,表单预填现有配置(别名锁定不可改),API 密钥留空则保持已保存的密钥不变;新增 GetCustomProvider 桥接读取配置(永不返回密钥)
- 修复重发请求丢失中断前思考记录:重试被引擎接受(TurnBegin)时,若被中断的一轮已有可见内容(思考/步骤),保留为已完结的历史轮次并清除错误标记,不再整对删除;空尝试维持原替换行为
...
- 修复模型选择被状态播报覆盖:composer 选定模型/思考等级后、发送前引擎状态事件仍携带旧值,会把选择器打回原模型(如切计划模式时跳回 K2.7);现对挂起选择加守卫,会话切换时以引擎值为准
- 计划审查对话框:计划文件改为直接在 VS Code 编辑器中打开(原 openFile 仅限工作区文件被静默拒绝),打开失败时回退内联预览,支持「重新打开」
