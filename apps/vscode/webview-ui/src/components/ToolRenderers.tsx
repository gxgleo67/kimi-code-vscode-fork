...
export type { TodoBlock } from "shared/legacy-sdk";
...
export function getTodoBlock(display?: DisplayBlock[]): TodoBlock | null {
  if (!display) {
    return null;
  }
  ...
  return (
    <div className="relative group/codeblock">
      ...
      {shouldCollapse && (
        ...
        >
          {expanded ? t("tools.less") : t("tools.expand", { count: lines.length - maxLines })}
        </button>
      )}
    </div>
  );
...
export function TodoStatusIcon({ status }: { status: string }) {
  if (status === "done") {
    return (
...
function TodoListTool({ result }: ToolRendererProps) {
  ...
  if (!todoBlock || !todoBlock.items || todoBlock.items.length === 0) {
    return <div className="py-2 text-xs text-muted-foreground">{!result?.is_error && t("tools.todoUpdated")}</div>;
  }
  return (
    <div className="py-1">
      ...
      {result && (
        <IORow label="OUT">
          {hasRichDisplay ? (
            ...
            <span className={cn("text-xs", !result.is_error ? "text-emerald-500" : "text-red-500")}>{!result.is_error ? t("tools.written") : formatOutput(result.output)}</span>
          )}
        </IORow>
      )}
    </div>
        ...
        <IORow label="OUT">
          {hasRichDisplay ? (
            ...
            <span className={cn("text-xs font-medium", !result.is_error ? "text-emerald-600 dark:text-emerald-500" : "text-destructive")}>
              {!result.is_error ? t("tools.replaced") : formatOutput(result.output)}
            </span>
          )}
        </IORow>
      )}
    ...
    <div className="divide-y divide-border">
      <IORow label="IN">
        <span className="text-[11px] font-mono">
          {pattern}
          {directory && <span className="text-muted-foreground ml-1.5">{t("tools.globIn", { directory })}</span>}
        </span>
      </IORow>
      {result && output && (
        <IORow label="OUT">
      ...
      {result && (
        <IORow label="OUT">
          {hasRichDisplay ? (
            ...
            <span className={cn("text-xs", !result.is_error ? "text-emerald-500" : "text-red-500")}>{!result.is_error ? t("tools.done") : t("tools.failed")}</span>
          )}
        </IORow>
      )}
    </div>
          ...
          <button onClick={() => setShowProcess(!showProcess)} className="flex items-center gap-1.5 text-[0.75rem] text-muted-foreground hover:text-foreground transition-colors">
            ...
            <span>
              {subagentSteps.length === 1
                ? t("tools.stepCount.one", { count: subagentSteps.length })
                : t("tools.stepCount.other", { count: subagentSteps.length })}
            </span>
          </button>
          {showProcess && (
            <div className="mt-2 space-y-3">
              {subagentSteps.map((step) => (
                <div key={step.n} className="space-y-2">
                  <div className="text-[0.75rem] text-muted-foreground uppercase tracking-wider">{t("tools.stepN", { n: step.n })}</div>
...
function getToolLabel(call: UIToolCall, t: ReturnType<typeof useT>): string {
  const args = parseArgs(call.arguments);
  switch (call.name) {
    case "Shell":
