...
describe('resolveRuntimeProvider api_key_env_var', () => {
  const ENV_VAR = 'KIMI_TEST_PROVIDER_API_KEY_ENV_VAR';

  function envRefConfig(provider: Record<string, unknown>): KimiConfig {
    return {
      defaultModel: 'custom/model',
      providers: { custom: { type: 'openai', ...provider } as KimiConfig['providers'][string] },
      models: {
        'custom/model': { provider: 'custom', model: 'custom-model', maxContextSize: 128_000 },
      },
    };
  }
  ...
  it('resolves the API key from the referenced process.env variable', () => {
    process.env[ENV_VAR] = 'sk-from-env-var';
    const resolved = resolveRuntimeProvider({
      config: envRefConfig({ apiKeyEnvVar: ENV_VAR }),
    });
    expect(resolved.provider).toMatchObject({ apiKey: 'sk-from-env-var' });
  });

  it('prefers a plaintext api_key over the env var reference', () => {
    process.env[ENV_VAR] = 'sk-from-env-var';
    const resolved = resolveRuntimeProvider({
      config: envRefConfig({ apiKey: 'sk-plaintext', apiKeyEnvVar: ENV_VAR }),
    });
    expect(resolved.provider).toMatchObject({ apiKey: 'sk-plaintext' });
  });

  it('falls back to the [env] subtable key when the referenced variable is unset', () => {
    const resolved = resolveRuntimeProvider({
      config: envRefConfig({ apiKeyEnvVar: ENV_VAR, env: { OPENAI_API_KEY: 'sk-from-subtable' } }),
    });
    expect(resolved.provider).toMatchObject({ apiKey: 'sk-from-subtable' });
  });

  it('prefers the env var reference over the [env] subtable key', () => {
    process.env[ENV_VAR] = 'sk-from-env-var';
    const resolved = resolveRuntimeProvider({
      config: envRefConfig({ apiKeyEnvVar: ENV_VAR, env: { OPENAI_API_KEY: 'sk-from-subtable' } }),
    });
  ...
  it('treats a blank referenced variable as unset', () => {
    process.env[ENV_VAR] = '   ';
    const resolved = resolveRuntimeProvider({
      config: envRefConfig({ apiKeyEnvVar: ENV_VAR }),
    });
    expect(resolved.provider).toMatchObject({ apiKey: undefined });
  });
});
